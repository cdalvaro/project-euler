...
BigInt::operator std::string_view() const {
    auto snumber = std::accumulate(begin(), end(), std::string{""},
                                   [](const auto &s, const auto &n) {
...
BigInt::operator size_t() const {
    ...
    size_t number;
    if (auto [p, ec] = std::from_chars(sv.data(), sv.end(), number);
        ec != std::errc()) {
